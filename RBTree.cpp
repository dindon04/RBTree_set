...
public:
    ...
    void buildTree() 
    {
        ...
        for (int i = 0; i != 10; i++) 
        {
            array.push_back(new Node(tempArr[i]));
            insert(root, array[i]);
        }
        display(root);
    }
...
public:
    ...
    bool contains(int x) 
    {
        for (const auto& elem : Set1.array) 
        {
            if (elem->val == x) 
            {
                return true;
...
int main() 
{
    ...
    Set newSet = {1, 2,7, 9, 10, 6, 3, 5, 8};

    newSet.insert_s(20);
    newSet.insert_s(15);
    newSet.insert_s(4);

    std::cout << "20? : " << (newSet.contains(20) ? "true" : "false") <<std::endl;
    std::cout << "4? : " << (newSet.contains(4) ? "true" : "false") << std::endl;
    std::cout << "all elements : ";
    newSet.display(newSet.root);
    return 0;
}